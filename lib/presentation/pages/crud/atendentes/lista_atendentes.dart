...
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:projeto_desafio_flutter/presentation/pages/crud/atendentes/edit_atendente.dart';
import '../../../../application/cubit/cadastro_cubit/cadastro_atendente_cubit/atendent_list/atendente_list_cubit.dart';

// lifecycle
class AtendentsList extends StatelessWidget {
  const AtendentsList({super.key});
        ...
        if (state is SuccessListAtendente) {
          return ListView.builder(
              ...
              itemBuilder: (context, i) {
                return ListTile(
                  subtitle: Text(
                    'Id: ${state.atendentList[i].id}',
                    style: const TextStyle(fontSize: 15),
                  ),
                  title: Row(children: [
                    const Icon(Icons.person),
                    const Spacer(flex: 1),
                    Text(state.atendentList[i].nome, style: const TextStyle(fontWeight: FontWeight.bold)),
                    const Spacer(flex: 1),
                    ...
                    InkWell(
                      onTap: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute<void>(
                              builder: (BuildContext context) => AtendenteEdit(idAtendente: state.atendentList[i].id)),
                        ).then((value) => context.read<AtendenteListCubit>().generateListAtendents());
                      },
                      child: Ink(
                          child: const Icon(
                        Icons.edit,
